...
void handle_inputs(vector<ball> &balls)
{
    ...
    if (selected_ball)
    {
        color current = selected_ball->fill_color;
        color new_col = {current.r, current.g, current.b, 1};

        fill_circle(new_col, selected_ball->ball_circle);
        draw_line(new_col, selected_ball->ball_circle.center, mouse_position());
    }
}
...
int main()
{
    const string WINDOW_NAME = "Tech demo: ball physics";

    open_window(WINDOW_NAME, 960, 540);
